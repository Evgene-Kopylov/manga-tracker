...
class TestMangaParser(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.driver = self.mp._driver(local=True)
        self.test_url = os.environ.get("MANGA_TRACKER_URL", 'http://127.0.0.1:8000') + '/test_page'

        page = Page()
        self.chapters = ['Chapter 4', 'Chapter 5']
        page.url = self.test_url + '?chapters=' + ', '.join(self.chapters)
        ...
        self.test_page = page
        session.add(self.test_page)
        session.commit()

    def tearDown(self) -> None:
        self.driver.quit()
        session.delete(self.test_page)
        session.commit()
